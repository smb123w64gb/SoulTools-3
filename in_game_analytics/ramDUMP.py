...
libipc.pine_read.restype = c_uint64
...
data = bytearray()
for x in range(32):#32 Mib
    print("Read %02iMib"%curMeg)
    curMeg+=1
    for y in range(0x20000):
        tmpChar = bytes(c_uint64(libipc.pine_read(ipc, offset, c_char(3), False)))
        offset+=8
        data.extend(tmpChar)
